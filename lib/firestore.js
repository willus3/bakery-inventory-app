  ...
  for (const item of weeklyPlan.items) {
    ...
    for (let i = 0; i < DAYS_ORDER.length; i++) {
      ...
      batch.set(woRef, {
        ...
        totalYield:             qty,
        scheduledStart:         `${dateStr}T${defaultStartTime}`,
        dueBy:                  `${dateStr}T${defaultDueTime}`,
        status:                 "planned",
        ingredientsRequired,
        ingredientsSufficient,
        insufficientIngredients,
...
// ─────────────────────────────────────────────────────────────────────────────
// SETTINGS
// A single document lives at settings/global. Using setDoc with merge: true
// means we can write only the fields that changed without wiping the rest.
// ─────────────────────────────────────────────────────────────────────────────

// The canonical defaults — used both here (as a fallback when no doc exists)
// and exported so SettingsContext can initialise state without a Firestore round-trip.
export const DEFAULT_SETTINGS = {
  bakeryName:         "Bakery MRP",
  defaultStartTime:   "06:00",
  defaultDueTime:     "10:00",
  adminEmails:        [],
};

// Reads the settings document and merges any missing fields with defaults.
// Returns the full settings object — callers always get every field.
export async function getSettings() {
  const ref  = doc(db, "settings", "global");
  const snap = await getDoc(ref);
  if (!snap.exists()) return { ...DEFAULT_SETTINGS };
  // Spread defaults first so any field added to DEFAULT_SETTINGS in the
  // future is automatically available even on old Firestore documents.
  return { ...DEFAULT_SETTINGS, ...snap.data() };
}

// Writes (or merges) settings into the settings/global document.
// Only the fields passed in `updates` are touched — everything else is preserved.
