...
// Creates a new demand plan document.
...
//   status, requiredBy, notes, createdBy }
...
//
// The batch contains N + 3 operations (N = number of ingredients):
...
export const createWorkOrderFromDemandPlan = async (demandPlan, currentUserEmail) => {
  ...
  // dueBy: default to 8 AM on the demand plan's required-by date.
  // requiredBy is stored as "YYYY-MM-DD"; appending "T08:00" produces a valid
  // datetime-local string the work order edit form can read and the baker can adjust.
  const dueBy = demandPlan.requiredBy ? `${demandPlan.requiredBy}T08:00` : "";
